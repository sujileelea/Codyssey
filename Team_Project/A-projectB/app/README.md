...
BBC News 코리아 뉴스를 **수집(RSS + 크롤링) → 정제 → AI 요약 → AI 인사이트 분석 → 시각화·리포트 → 내보내기**까지
하나의 CLI로 처리하는 Python 애플리케이션이다. 저장소는 SQLite, AI는 Gemini 무료 티어(`gemini-3.5-flash-lite`)를 쓴다.
...
| 팀원 | 파트 | 모듈 |
|---|---|---|
| owner(수지) | CLI·설정·로깅·수집·리포트·내보내기·통합 | `main.py` `config.py` `log.py` `collector/` `report/` |
...
API 키는 코드나 `config.json`에 쓰지 않는다. `config.json`의 `ai.api_key_env`에는 **환경변수 이름**만 있고,
값은 `.env` 또는 셸 환경변수에서 읽는다. `.env`·`data/`·`logs/`는 git에 올라가지 않는다.

키 없이 파이프라인만 확인하려면 `config.json`의 `ai.provider`를 `"mock"`으로 바꾼다.
...
`analyze`는 **요약이 있는 기사만** 정성 분석에 넣으므로 `summarize`를 먼저 실행한다.

## 서브커맨드

| 명령 | 주요 옵션 | 설명 |
|---|---|---|
| `fetch` | `--source {rss,crawl,all}` `--limit N` `--category C` `--no-body` | 방법 1(RSS) + 방법 2(크롤링) 수집 → `raw_news`. RSS 건은 기사 페이지를 크롤링해 본문을 보강한다 |
| `clean` | `--duplicate-policy {skip,upsert}` `--all` `--limit N` | 필수 필드 검증·텍스트 정규화·날짜 통일·결측 처리 → 중복 정책 → `news` |
| `summarize` | `--unsummarized`(기본) `--all` `--id ID…` `--limit N` `--force` | Gemini 요약 → `news.summary`. 기요약 건은 스킵, 실패는 로깅 후 다음 건 |
| `analyze` | `--date-from` `--date-to` `--category` `--limit` `--sort` `--show ID` `--list` | 조건별 기사를 20건 배치로 분석하고 종합 → `insights` (트렌드 3·키워드 6·주요 이슈 4) |
...
공통: `--config PATH`, `--verbose`. 종료 코드 0 성공 · 1 부분 실패 · 2 설정/인자 오류.
...
전체 실행 로그와 산출물은 `../제출물/실행 결과.md` 참고.
...
- RSS 건의 본문은 기사 페이지 크롤링으로 보강하고, 카테고리는 정제 단계에서 같은 URL의 크롤링 raw 행을 참조해 채운다(없으면 `기타`).
- RSS 링크의 추적 파라미터(`at_medium=RSS…`)는 URL 정규화로 제거해 크롤링 건과 같은 URL로 중복 처리된다.
- 기사 본문의 CSS class는 해시형이라 구조 선택자(`main p`)만 쓰고 `figure`·바이라인·캡션은 제외한다.

### 크롤링 정책 준수

- `robots.txt`(2026-09-09 확인): `/korean/` 경로에 차단 규칙 없음.
- 요청 간 최소 1.5초 지연(`http.delay_seconds`), 타임아웃 10초, 재시도 2회, 식별 가능한 User-Agent.
- 기본 `--limit 10`(섹션당) → 전체 수집 시 약 60회 요청. 학습 목적의 소량 수집만 한다.
...
| `news` | 정제 완료(clean). `url` UNIQUE가 중복 키. 요약(`summary`·`summary_model`·`summarized_at`)도 여기 | 혜민·진성 |
| `insights` | 인사이트 분석 결과(정량 JSON·배치 JSON·최종 JSON) | 경학 |
| `sentiment_results` | 기사별 감성 (`news_id` UNIQUE, upsert) | 경학 |

raw와 clean을 분리하는 이유: 정제 규칙이 바뀌거나 버그가 있어도 raw에서 다시 정제할 수 있고(`clean --all`),
하류 단계는 clean만 읽으므로 데이터 계약이 단순해진다.

### 정제 규칙과 중복 정책

- 필수 필드: 제목·URL·본문. 하나라도 없으면 WARNING 후 탈락(raw는 보존).
- 정규화: HTML 엔티티·태그 제거, 공백 정리, URL 추적 파라미터 제거.
- 날짜: RFC 2822 / ISO 8601 / `YYYY.MM.DD` 등 → `YYYY-MM-DD HH:MM:SS`(KST). 발행일이 없으면 수집 시각으로 대체.
- 결측: 소스 → `unknown`, 카테고리 → `기타`.
- `skip`(기본): 같은 URL이 있으면 저장하지 않음. `upsert`: 메타를 갱신하되 **본문은 더 긴 쪽을 유지**하고, 본문이 실제로 바뀐 경우에만 요약을 초기화한다.

### AI 호출

- Gemini `gemini-3.5-flash-lite` (google-genai SDK). 인사이트·감성은 JSON Schema 응답, 요약은 자유 텍스트.
- 무료 티어 분당 요청 한도를 지키려고 호출 간격 4.5초를 두고, 429가 오면 서버가 알려준 시간만큼 기다렸다가 재시도한다(`ai.min_interval_seconds`, `ai.rate_limit_retries`).
- 요약·감성은 건별 실패를 로깅하고 다음 건을 계속한다. 분석은 배치 단위 재시도 후 실패하면 중단한다.
- `provider: mock`이면 키 없이 동작한다(테스트·파이프라인 점검용).
...
- 차트(PNG, 한글 폰트 자동 탐지 — macOS AppleGothic / Windows Malgun Gothic / Linux NanumGothic): 카테고리별 뉴스 수, 일자별 수집 추이, 일자별 발행 추이, 감성 분포.

## 정기 실행 (cron)

매일 08:00·18:00에 수집 → 정제 → 요약 → 감성 → 리포트를 돌리는 예시. 경로는 실제 위치로 바꾼다.
...
# 주 1회 인사이트 분석 (월요일 09:00, 최근 7일)
0 9 * * 1  cd /path/to/A-projectB/app && .venv/bin/python main.py analyze --date-from "$(date -v-7d +\%F)" --date-to "$(date +\%F)" >> logs/cron.log 2>&1
```

- cron은 `.env`를 자동으로 읽지 않지만 이 프로그램은 `config.py`가 `app/.env`를 직접 읽으므로 그대로 동작한다. 환경변수로 주려면 crontab 상단에 `GEMINI_API_KEY=...`를 둔다.
- `%`는 cron에서 특수문자라 `\%`로 이스케이프한다. Linux `date`는 `-v-7d` 대신 `-d '7 days ago'`.
- 같은 URL은 `skip` 정책으로 걸러지므로 하루 두 번 돌려도 중복 저장되지 않는다.
- Windows는 작업 스케줄러에서 `python main.py fetch …`를 실행하는 작업을 같은 순서로 등록한다.
