...
- `app/` CLI를 **BBC News 코리아 실데이터 + Gemini 무료 티어**로 수집부터 내보내기까지 끝까지 실행한 기록 (2026-09-09)
	- 명령·로그는 `app/logs/app.log` 발췌, 산출 파일은 `app/output/`에 그대로 보관
	- AI 응답 원문은 `app/data/news.db`(비커밋)에 저장
...
| RSS(`collection_method=rss`) | **10건** — 8건은 크롤링과 같은 기사(본문 재요청 생략), 2건은 본문 보강 |
| 소요 | 약 1분 30초 (HTTP 요청 58회 — 섹션 5 · 기사 50 · RSS 1 · 본문 보강 2) |
...
- **중복 14건** = 여러 섹션에 동시에 실린 기사(북한 섹션 10건 중 5건이 국내·세계와 겹침) + RSS·크롤링 중복
	- RSS 링크의 `?at_medium=RSS&at_campaign=rss`는 URL 정규화로 제거 → 크롤링 건과 같은 URL로 판정
- **카테고리 분포**: 국내 10 · 문화 10 · 건강·과학 10 · 세계 9 · 북한 5 · 기타 2(RSS에만 있던 기사, 섹션 정보 없음)
- **날짜 통일**: RSS(RFC 2822, GMT)와 JSON-LD(ISO, UTC) 두 형식 모두 `YYYY-MM-DD HH:MM:SS`(KST)로 변환

---

## 3. summarize — AI 요약

- 첫 실행은 초기 설정 모델 `gemini-2.5-flash-lite`가 **신규 사용자에게 종료(404)**, 두 번째는 무료 티어 **분당 15회 한도(429)** 에 도달
	- 두 경우 모두 "실패 로깅 후 스킵" 동작 확인
	- 조치: 모델을 `gemini-3.5-flash-lite`로 교체, 호출 간격 4.5초 + 429 대기 재시도 추가 → 이후 전건 성공
...
$ python main.py summarize --unsummarized --limit 60         # 호출 간격 4.5초 + 429 대기 재시도 적용 후
...
- **최종**: 46/46건 요약, 평균 3,588자 → 238자 (압축률 6.6%)
...
- **분포**: 부정 20건(43.5%) · 중립 14건(30.4%) · 긍정 12건(26.1%)
- **소요**: 약 6분 (호출 간격 4.5초 × 46건)
...
| AI 한도 초과(429) | `summarize --all` 43건 연속 | 17건 성공 후 429 → 26건 스킵 → 호출 간격·대기 재시도 적용 후 26/26 성공 |
| API 키 없음 | `.env` 제거 후 `summarize` | `[ERROR] 실행 실패: 환경변수 GEMINI_API_KEY에 … 설정되어 있지 않습니다`, 종료 코드 1 — `list`·`report`·`export`는 키 없이 정상 |
| 필수 필드 결측 | 본문 없는 raw | `[WARNING] raw_id=… 정제 실패: 본문이 없습니다.` 후 계속 |
| 중복 정책 | 같은 raw를 `skip` / `upsert --all` | skip: 중복 14건 · upsert: 수정, 본문 동일 시 요약 보존 |
...
- 요약 흐름(스킵·강제·API 실패) · 인사이트 배치/검증 · 감성 검증 (Mock 클라이언트)
...
| AI 요약 `--all/--id/--unsummarized` · 실패 스킵 · 기요약 스킵 | `summarizer.py`, §3 로그 |
| 인사이트 2항목 이상 · 저장·조회 | 트렌드·키워드·주요 이슈 → `insights`, `analyze --show/--list` |
| matplotlib 2종 + 한글 + PNG | PNG 4종, AppleGothic 적용 (§6) |
...
| 영구 저장소 · 모듈 4개 이상 | SQLite · Python 모듈 19개 (`app/` 12 + `collector/` 4 + `report/` 3) |
| 보너스: list/show 필터·페이지 · 감성 분석+시각화 · cron 안내 | §8 · §5 + `sentiment_counts.png` · `app/README.md` §정기 실행 |
