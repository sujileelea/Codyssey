...
flowchart LR
    ...
    H --> C["Gemini 3.5 Flash-Lite<br/>분류·요약·AI News 초안 (JSON)"]
...
| 5 | Gemini 3.5 Flash-Lite | **Action(보너스 1)**. 분류·태그·3줄 요약·AI News 글 초안을 구조화 JSON으로 생성 | 프롬프트 §4 |
...
| 중간 | 본문 마크다운(제목 줄 `Title:` + 본문), Gemini JSON `{title, category, tags[], summary, article_md, confidence}` |
...
- _구현 후 기입_: 활성(Published) 상태, 실행 이력 캡처, Activepieces·AI Studio 무료 한도 화면.
