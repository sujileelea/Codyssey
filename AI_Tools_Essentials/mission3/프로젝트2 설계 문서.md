...
> 구현·발행 완료(초안 2026-08-26, 모델 결정·성공 경로 실측 2026-08-27).
...
flowchart LR
    ...
    B -- 예 --> M["Code<br/>파일명 정리·출처 추가·Base64"]
    M --> G["GitHub – vault-lealoft<br/>Articles/AI News/제목.md 생성"]
...
| 5 | JSON 파싱 | Gemini 응답의 `body.candidates[0].content.parts[0].text`를 분기에서 쓸 객체로 변환 | `{title, category, tags, summary, article_md, confidence}` |
| 6 | 분기 | **Router**. `confidence`를 AI·기술 뉴스 확신도로 정의하고 높은 경우에만 발행 경로 | `confidence > 0.69` |
| 7 | 파일 준비 | 제목에서 파일명 금지 문자를 제거하고 본문 말미에 출처·자동 초안 표기를 붙여 Base64로 변환 | `Articles/AI News/<title>.md` |
| 8 | GitHub | **Action**. 공개 저장소에 글을 커밋하면 배포 시스템이 사이트에 반영 | `PUT /repos/sujileelea/vault-lealoft/contents/{path}` |
| 9 | Slack 답글(발행 경로) | 발행 완료 안내 + 제목·요약·사이트 링크를 원 메시지 스레드에 전송 | — |
| 10 | Slack 답글(보류 경로) | 제목·분류·확신도·요약만 스레드에 전송 | — |
...
| 출력 1 | `vault-lealoft/Articles/AI News/<title>.md` (public, 사이트 소스) |
| 출력 2 | Slack 스레드 답글 |

Lea-Loft 글 형식은 frontmatter 없는 순수 마크다운이며 제목 = 파일명이다(기존 AI News 글과 동일). 글 말미에 출처 링크와 "자동 생성 초안" 표기를 넣는다. 공유 Vault는 동시 쓰기 안전 규칙상 `Vault/.ops/vault-git`만 커밋할 수 있으므로 Activepieces가 private Vault에 직접 쓰지 않는다. 자동화는 공개 배포 저장소만 갱신한다.
...
| 현재 제한 | 같은 제목의 파일을 다시 처리하면 contents API가 `sha` 없는 create 요청을 거절 | 중복 발행은 막히지만 갱신(upsert)은 별도 보완 필요 |
